...
class UserMeCacheView(APIView):
    permission_classes = [IsAuthenticated]

from .serializers import RegisterSerializer, LoginSerializer, UserSerializer, EmailTokenObtainPairSerializer
...
class EmailTokenObtainPairView(TokenObtainPairView):
    """
    Custom token obtain view that works with email-based authentication.
    This extends the default TokenObtainPairView to handle email login.
    """
...
class MeCacheView(APIView):
    ...
    def get(self, request):
        ...
        if cached_user is not None:
            return Response({
                "cached": True,
                "data": UserSerializer(cached_user).data
            })
        else:
            cache.set(cache_key, user, timeout=settings.CACHE_TTL)
            return Response({
